...
auto mult_threading(std::vector <std::vector <int>>& first, std::vector <std::vector <int>>& second,
	...
	std::vector <std::vector <std::unique_ptr <std::mutex>>> mutexes(block_count);
	...
	while (true) { //переход к след строке левой матрицы
		...
		while (true) { //проход по строке блоков левой матрицы
			...
			while (true) { //перемножение блока из левой матрицы на все требуемые из правой
				threads.emplace_back(block_mult, std::ref(first), std::ref(second), first_left_corner.x, first_left_corner.y, first_right_corner.x, first_right_corner.y,
					second_left_corner.x, second_left_corner.y, second_right_corner.x, second_right_corner.y, std::ref(result), 
					std::ref(*mutexes[first_left_corner.x / block_size][second_left_corner.y / block_size]));
