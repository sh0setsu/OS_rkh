...
void block_mult(std::vector <std::vector <int>>& first, std::vector <std::vector <int>>& second, 
        ...
        std::vector <std::vector <int>>& result, pthread_mutex_t* m) {
    /*if (y2 - y1 != x4 - x3) {
...
void* block_mult_wrapper(void* arg) {
    ThreadArgs* args = static_cast<ThreadArgs*>(arg);
    block_mult(*args->first, *args->second,
               args->x1, args->y1, args->x2, args->y2,
               args->x3, args->y3, args->x4, args->y4,
               *args->result, args->mutex);
    delete args;
    return nullptr;
}
...
auto mult_threading(std::vector <std::vector <int>>& first, std::vector <std::vector <int>>& second,
    ...
    std::vector<std::vector<pthread_mutex_t*>> mutexes(block_count);
    for (size_t i = 0; i < block_count; ++i) {
        mutexes[i].resize(block_count);
        for (size_t j = 0; j < block_count; ++j) {
            mutexes[i][j] = new pthread_mutex_t;
    ...
    while (true) { //переход к след строке левой матрицы
        ...
        while (true) { //проход по строке блоков левой матрицы
            ...
            while (true) { //перемножение блока из левой матрицы на все требуемые из правой
                ...
                pthread_t thread;
                pthread_create(&thread, NULL, block_mult_wrapper, args);
